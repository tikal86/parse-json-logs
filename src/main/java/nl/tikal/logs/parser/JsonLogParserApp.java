...
import nl.tikal.logs.parser.view.GanttTab;
import nl.tikal.logs.parser.view.StatusPanel;
...
public class JsonLogParserApp extends Application {
    ...
    public void start(final Stage primaryStage) {
        ...
        // Bottom: status bar
        root.setBottom(StatusPanel.getInstance());
    ...
    private void loadFile(final File file) {
        try {
            StatusPanel.changeStatus("Loading file: " + file.getName() + "...");
            
            final var jsonObjects = parser.parseLogFile(file.getAbsolutePath());
            
            if (jsonObjects.isEmpty()) {
                StatusPanel.changeStatus("No valid JSON objects found in file.");
            ...
            StatusPanel.changeStatus("Loaded " + jsonObjects.size() + " entries from " + file.getName() + tableTab.getColumnStatus());
            
            // Build Gantt chart
            ganttTab.buildGanttChart();
            
        } catch (final IOException ex) {
            StatusPanel.changeStatus("Error loading file: " + ex.getMessage());
    ...
    private void clearTabs() {
        ...
        StatusPanel.changeStatus("Table cleared. Load a new file to continue.");
    }
    ...
    private void showColumnSelector() {
        ...
        okButton.setOnAction(event -> {
            ...
            final var visibleCount = checkBoxMap.values().stream()
                                                ...
                                                .count();
            StatusPanel.changeStatus("Showing " + visibleCount + " of " + tableTab.getColumns().size() + " columns");
        });
